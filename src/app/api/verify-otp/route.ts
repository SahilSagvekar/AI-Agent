...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!record) return NextResponse.json({ message: "OTP not found, request again" }, { status: 400 });
    if (new Date() > record.expiresAt) {
      await prisma.emailOtp.delete({ where: { email } });
      return NextResponse.json({ message: "OTP expired, request again" }, { status: 400 });
    }
    if (record.otp !== otp)
    ...
    return NextResponse.json({ message: "User registered", user: { email, name: `${firstName} ${lastName}` } });
  } catch (err) {
    console.error(err);
