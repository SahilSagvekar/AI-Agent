...
// // ✅ Helper function: Get area code for ZIP
// async function getAreaCodeForZip(zip: string): Promise<string> {
//   const res = await fetch(`https://api.api-ninjas.com/v1/zipcode?zip=${zip}`, {
//     headers: { "X-Api-Key": process.env.NINJAS_API_KEY! },
//   });
...
// // ✅ Helper function: Provision Twilio Number
// async function provisionTwilioNumber(userId: number) {
//   const location = await prisma.laundromatLocation.findFirst({
//     where: { userId: userId },
//     select: {
//       zipCode: true,
...
//   const areaCode = await getAreaCodeForZip(location.zipCode);
...
//   return "8282696969"; // dummy number for now
// }
...
//       // Retrieve full subscription to avoid TS error
...
//         console.log(`Subscription payment succeeded for user ${payment.userId}`);

//         // ✅ Count invoices to check if 3 payments are done
//         const invoices = await stripe.invoices.list({
//           subscription: subscriptionId,
//           limit: 10,
//         });
...
//                 price: process.env.STRIPE_NORMAL_PRICE_ID!, // $175 + $45/location plan
//               },
//             ],
//             proration_behavior: "create_prorations", // ✅ ensures partial month is billed fairly
//           });
//         }
//       }
//     }
...
//   // ✅ Handle One-Time Payment (Add location / Buy number)
//   if (event.type === "payment_intent.succeeded") {
//     console.log("payment-successfull (one-time)");
//     const paymentIntent = event.data.object as Stripe.PaymentIntent;
//     const userId = paymentIntent.metadata.userId;
//     const flowType = paymentIntent.metadata.flowType;
